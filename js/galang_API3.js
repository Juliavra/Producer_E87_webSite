...
async function procesarResultado(item, type) {
    ...
    try {
        if (type === "artist" || type === "artistSearch") {
            ...
            await verLabel(item.resource_url, "resultsList");
...
function detalleRestanteMaster(data, div, btn) {
    if (btn) btn.remove();

    if (data.notes) {
        const notes = document.createElement("p");
        notes.innerHTML = `<strong>Notes:</strong><br>${data.notes}`;
        div.appendChild(notes);
    }
    // Aquí podrías agregar géneros, videos, o lo que falte.
}
...
async function obtenerDetalleArtista(artistId) {
    ...
    try {
        ...
        printsArtistaReleases(data.id, "containerReleases");
    } catch (error) {
        console.error(error);
...
function printsRelease(data) {console.log("DATA: " + data)
    const container = document.getElementById('container');
    const newDiv = document.createElement('div');
    newDiv.style.border = "2px solid green";
    newDiv.style.padding = "15px";
...
async function printsArtistaReleases(data, containerId = "containerReleases") {
    const token = document.getElementById("myToken").value.trim();
    ...
    try {
        ...
        if (!response.ok) throw new Error(`Error HTTP: ${response.status}`);
        const data = await response.json();
        //console.log(`"printsArtistasRelease  "  (${data.year || 'N/A'})"\n" ID: ${data.id}"\n"`);
