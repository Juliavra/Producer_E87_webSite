...
osc.frequency.value = "C0";                     // start at "C4"
//osc.frequency.rampTo("C2",1 );                  // ramp to "C2" over 2 seconds
/**/
...
/*
//osc.start().stop("+1.065");                        // start the oscillator for 2 seconds
osc.start("+1.125").stop("+1.190");*/
...
//osc.start("+2.25").stop("+2.315");
osc.frequency.value = "C0"; 

//osc.start().stop("+2.065");                        // start the oscillator for 2 seconds
//osc.start("+2.125").stop("+2.190");
//osc.start("+2.25").stop("+2.315");
osc.start("+2.375").stop("+2.440");
osc.start("+2.50").stop("+2.565");
osc.start("+2.625").stop("+2.690");
osc.start("+2.75").stop("+2.815");
//osc.start("+2.875").stop("+2.940");
...
//Tone.getTransport().bpm.rampTo(800, 3);
...
}, ["C4", ["E4", "D4"], "E4", "G5",["G6", "A6"],"E2","E2","E2","E1","E1","E0","E2","G2","A2"]).start(0).stop(10);
...
}, snares).start(0).stop("+14");
/**/
const snarePart2 = new Tone.Part(function(time) {
  snareDrum.triggerAttackRelease('32n', time)
}, snares).start(14).stop("+30");
}
